...
def test_todo_crud_operations(client: TestClient, auth_headers):
    ...
    create_response = client.post(
        "/todos/", headers=auth_headers, json={"description": "Test todo item"}
    )
...
def test_todo_authorization(client: TestClient, auth_headers):
    # Create a todo
    create_response = client.post(
        "/todos/", headers=auth_headers, json={"description": "Test todo"}
    )
...
def test_todo_not_found(client: TestClient, auth_headers):
    ...
    response = client.put(f"/todos/{non_existent_id}/complete", headers=auth_headers)
    assert response.status_code == 404
